    ...
    tabPanel("Continuous",
             column(2,
                    ...
                    fluidRow(textInput("d2mean", "Difference (d)",
                                       value = "", width = "100%"),
                             bsTooltip("d2mean", "Expected difference.")),
                    fluidRow(textInput("m2mean", "Clusters per arm (m)",
                                       value = "3 6 9 12", width = "100%"),
                             bsTooltip("m2mean", "Number of clusters per arm.")),
                    fluidRow(textInput("n2mean", "Cluster size (n)",
                                       value = "100 300 200", width = "100%"),
                             bsTooltip("n2mean", 'The mean number of individuals per cluster. If the cluster sizes are all identical, then the "cv" parameter should be set to 0.')),
                    fluidRow(textInput("icc2mean", "ICC (icc)",
                                       value = ".001 .005 .01", width = "100%"),
                             bsTooltip("icc2mean", 'The intracluster correlation coefficient.')),
                    fluidRow(textInput("varw2mean", "Within variance (varw)",
                                       value = "1", width = "100%"),
                             bsTooltip("varw2mean", 'The within-cluster variance, assumed to be the same for all clusters.')),
                    fluidRow(textInput("cv2mean", "Cluster size CV (cv)",
                                       value = "0", width = "100%"),
                             bsTooltip("cv2mean", 'The coefficient of variation of cluster sizes. If cluster sizes are all equal, then this parameter should be 0.')),
                    fluidRow(checkboxGroupInput("method2mean", "Unequal Cluster Size Adjustment",
                                                choices = c(Taylor = "taylor", Weighted = "weighted"),
                                                selected = "taylor"),
                             bsTooltip("method2mean", 'Method for calculating the effect of unequal cluster sizes. Neither method has an effect if "cv" above is 0.')),
                    fluidRow(
    ...
    tabPanel("Binary",
             column(2,
                    ...
                    fluidRow(
                      ...
                      bsTooltip("p1inc2prop", 'Select if "p1" or "p2" is blank and the treatment proportion is thought to be larger than the control proportion.'),
                      column(6,checkboxInput("pooled2prop", "Pooled")),
                      bsTooltip("pooled2prop", 'Select if a a pooled test statistic is desired.')
                    ),
                    ...
                    fluidRow(textInput("cv2prop", "Cluster size CV (cv)",
                                       value = "0", width = "100%"),
                             bsTooltip("cv2prop", 'Used to account for variable cluster sizes. Has no effect if "cv" is set to 0.')),
                    fluidRow(
...
server <- function(input, output, session){
  ...
  res2mean <- eventReactive(
    input$calc2mean,
    {
      ...
      mutate_if(tab, !(names(tab) %in% c(target,"error")), factor)
    })
